 ...
 * Q (Stop mode that turns off the pumps!)
 (T)
 *TAIL MODE Using servo and pump adjustable hz, delay, on off, and flow rate
 *
 *FLOW MODE (F)
 *just does the pump
...
const int pumpLPin =  6;// the number of the pump pin
const int pumpRPin =  5;// the number of the pump pin
int pumpL_on = 0;
int pumpR_on = 1;
const String valid_params = "shd";//Speed, Hertz
float s = 0; //Speed of the pump 
float h = 1; //Hertz of the pump/tasil
float d = 0; //Delay to turn off pump
int pump_val = 255*s;
...
void mode_helper(){
  ...
  mode_sw = true;
  modeMillis = millis();
  Serial.println("Diabling motors during a mode switch");
}
